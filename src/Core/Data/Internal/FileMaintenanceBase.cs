...
{
    /// <summary>
    /// Common file system maintenance implementation
    /// </summary>
    internal abstract class FileMaintenanceBase : IDisposable
    {
        /// <summary>
        ...
        protected FileMaintenanceBase(string repositoryPath, bool loggingEnabled)
        {
        ...
        /// <param name="loggingEnabled">Indicates if the maintenance process should log its actions.</param>
        protected FileMaintenanceBase(string repositoryPath, string productName, string applicationName, int maxAgeDays, int maxSizeMegabytes, bool loggingEnabled)
        {
            if (string.IsNullOrEmpty(repositoryPath))
            {
                throw new ArgumentNullException(nameof(repositoryPath));
        ...
        protected virtual void OnFileRemoved(string fileNamePath)
        {

        }

        /// <summary>
        /// Get file information for all files that can be pruned
        /// </summary>
        /// <returns></returns>
        protected abstract List<FileInfo> OnGetEligibleFileInfo();
        ...
        private bool ProcessPruneForAge(int maxAgeDays, int maxSizeMegabytes, out bool capacityPruningRequired)
        {
            ...
            var fileFragments = OnGetEligibleFileInfo();
        ...
        private bool RemoveSessionFragment(FileInfo victimFile)
        {
            ...
            try
            {
                ...
                if (fileRemoved)
                    OnFileRemoved(victimFile.FullName);
            }
            catch (Exception ex)
            {
                GC.KeepAlive(ex);
        ...
        private static int SortFilesByUpdateDateDesc(FileInfo leftFile, FileInfo rightFile)
        {
            ...
            if (returnVal == 0)
                returnVal = leftFile.CreationTimeUtc.CompareTo(rightFile.CreationTimeUtc);
